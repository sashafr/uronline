...
class AdminAdvSearchForm(forms.Form):
    ...
    cp1 = forms.ModelChoiceField(label='', required=False, queryset=DescriptiveProperty.objects.filter(control_field = True).filter(Q(primary_type='SO') | Q(primary_type='AL')))
    ...
    cp2 = forms.ModelChoiceField(label='', required=False, queryset=DescriptiveProperty.objects.filter(control_field = True).filter(Q(primary_type='SO') | Q(primary_type='AL')))
    ...
    cp3 = forms.ModelChoiceField(label='', required=False, queryset=DescriptiveProperty.objects.filter(control_field = True).filter(Q(primary_type='SO') | Q(primary_type='AL')))
    ...
    fp1 = forms.ModelChoiceField(label='', required=False, queryset=DescriptiveProperty.objects.filter(Q(primary_type='SO') | Q(primary_type='AL')))
    ...
    fp2 = forms.ModelChoiceField(label='', required=False, queryset=DescriptiveProperty.objects.filter(Q(primary_type='SO') | Q(primary_type='AL')))
    ...
    fp3 = forms.ModelChoiceField(label='', required=False, queryset=DescriptiveProperty.objects.filter(Q(primary_type='SO') | Q(primary_type='AL')))
...
class SubjectLocationAdminForm(ModelForm):
    location = LocationChoices(
        label = Location._meta.verbose_name.capitalize(),
        widget = AutoHeavySelect2Widget(
            select2_options = {
...
class SubjectMediaAdminForm(ModelForm):
    media = MediaChoices(
        label = Media._meta.verbose_name.capitalize(),
        widget = AutoHeavySelect2Widget(
            select2_options = {
...
class SubjectPersonOrgAdminForm(ModelForm):
    person_org = PersonOrgChoices(
        label = PersonOrg._meta.verbose_name.capitalize(),
        widget = AutoHeavySelect2Widget(
            select2_options = {
...
class FileSubjectAdminForm(ModelForm):
    rsid = FileChoices(
        label = File._meta.verbose_name.capitalize(),
        widget = AutoHeavySelect2Widget(
            select2_options = {
...
class SubjectPropertyInline(admin.TabularInline):
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == 'property':
            kwargs["queryset"] = DescriptiveProperty.objects.filter(Q(primary_type='SO') | Q(primary_type='AL')).exclude(control_field = True)
...
class MediaSubjectRelationsInline(admin.TabularInline):
    ...
    suit_classes = 'suit-tab suit-tab-relations'
    extra = 1
    form = SubjectMediaAdminForm

class SubjectPersonOrgRelationsInline(admin.TabularInline):
    model = SubjectPersonOrgRelations
    ...
    suit_classes = 'suit-tab suit-tab-relations'
    extra = 1
    form = SubjectPersonOrgAdminForm
...
class SubjectAdmin(admin.ModelAdmin):
    ...
    inlines = [SubjectPropertyInline, SubjectControlPropertyInline, MediaSubjectRelationsInline, LocationSubjectRelationsInline, SubjectPersonOrgRelationsInline, FileInline, SubjectCollectionEntityInline, SubjectLinkedDataInline]
    ...
    def save_formset(self, request, form, formset, change):
        ...
        for instance in instances:
            if isinstance(instance, SubjectProperty) or isinstance (instance, SubjectControlProperty) or isinstance (instance, MediaSubjectRelations) or isinstance (instance, LocationSubjectRelations) or isinstance (instance, SubjectPersonOrgRelations):
                instance.last_mod_by = request.user
            instance.save()                
    ...
    def get_search_results(self, request, queryset, search_term):
        ...
        med = adv_fields['med']
        if med != '':
            queryset = queryset.filter(mediasubjectrelations__media_id=med)            
